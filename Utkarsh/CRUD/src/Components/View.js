...
const View = () => {
    ...
    const handeldelete=async(id)=>{
        await axios.delete('http://localhost:4000/posts/'+id).then((res)=>alert("record Deleted"));
        getUser();
    };
    ...
    const [UpdateData,SetUpdateData]=useState({
      ...
      const handleUpdate=async()=>{
        await axios.put(`http://localhost:4000/posts/${UpdateData.id}`, UpdateData).then((res)=>alert("record Updated"));
        getUser();
        window.location.reload();
    };
